...
describe('extractContractVersion', () => {
  ...
  it('[fork] keeps the -tron suffix, so the audit lookup hits the fork entry', () => {
    expect(
      extractContractVersion(
  ...
  it('[fork] keeps a -tron revision suffix', () => {
    expect(
      extractContractVersion(
        '/// @custom:version 1.0.0-tron-r2\ncontract Foo {}'
      )
    ).toBe('1.0.0-tron-r2')
  })

  it('[fork] stops at the end of the tag rather than swallowing the next line', () => {
    expect(
      extractContractVersion('/// @custom:version 1.0.0-tron\ncontract Foo {}')
    ).toBe('1.0.0-tron')
  })
})
