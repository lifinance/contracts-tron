...
export async function deletePendingProposals(
  ...
  for (const h of hashes) {
    const doc = await pendingTransactions.findOne({ network: String(network), safeTxHash: String(h) })
    if (!doc) {
    ...
    const res = await pendingTransactions.deleteOne({ network: String(network), safeTxHash: String(h) })
    consola.success(
