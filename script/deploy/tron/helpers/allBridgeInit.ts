...
export function parseAllBridgeMappings(
  ...
  if (!Array.isArray(mappings) || mappings.length === 0)
    throw new Error(
      'config/allbridge.json has no non-empty "mappings" array — AllBridgeFacet v2.2.0+ seeds its chain-id mappings from it. Is the fork synced to an upstream main that includes it?'
    )
