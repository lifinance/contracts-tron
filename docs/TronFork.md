...
`contracts-tron` fork and how to keep it in sync with `contracts`. **Part 2**
is the operational how-to for deploying contracts to Tron. For general
Tron-vs-EVM technical reference (address formats, fees, TronWeb code
...
`contracts-tron` is a **true GitHub fork** of `lifinance/contracts` that
exists solely so Tron can ship a USDT-safe token-transfer path. It carries a
small, deliberate delta on top of `main`; it is an **overlay, not an
...
1. In **`main`**: standardize ERC-20 transfers to route through `LibAsset`
   (removing scattered direct `safeTransfer` calls) — a codebase-consistency
   win that is _not_ Tron-specific and is now CI-enforced by
   `enforceLibAssetRouting.yml`. `main` contains **no** Tron branching.
...
If you find yourself adding anything beyond this shape to the fork, stop and
reconsider — it almost certainly belongs in `main`.

### Repo roles — when to use which

|                        | `lifinance/contracts` (upstream)                             | `lifinance/contracts-tron` (fork)                              |
| ---------------------- | -------------------------------------------------------------- | ------------------------------------------------------------- |
...
**Direction:** one-way, `contracts` → `contracts-tron`. The fork pulls
upstream; it never pushes back to `main`.

**Current process (PR-based):** a periodic sync PR is opened on a
`sync/upstream-YYYY-MM-DD` branch that merges `upstream/main` into the
fork's `main`. Review the (usually tiny) _real_ diff, then merge. Recent
examples: `contracts-tron` #13 (May) and #15 (June, 192 commits). Merge
conflicts are rare because the fork's delta is so small.

**When to update which — the order:**

1. **Feature / fix** → land it in **`contracts` `main`** first (normal PR +
   audit + merge).
2. **Tron deploy** → sync the fork from `main`, ensure the `-tron` delta is
   intact, then deploy from the fork's `main`. Deploy logs from that deploy
   are PR'd to **upstream** `contracts` (not the fork) and then sync back
   down — see [Part 2](#part-2--deploying-to-tron).
3. **A contract that has a `-tron` variant** → update **both** repos,
   `main` first, then the fork (bumping the `-tron` version).
...
The biggest recurring headache: **required checks and network healthchecks
fire on sync PRs and block them on failures that have nothing to do with
the fork.**
...
`skipHealthcheck` flag for `somnia` in `config/networks.json` (temporary).
...
bypass list for `main`. A scheduled job:

1. Fetches `upstream/main` and merges it into the fork's `main` locally.
2. **No conflicts** → pushes the merge commit **directly** to `main` — no
   PR, no required checks. Branch protection still applies to humans; only
   the app bypasses.
...
`feat/sync-upstream-workflow` branch.

### New-dev gotchas (quick checklist)

- **Never** develop features in `contracts-tron`. Everything starts in
  `contracts`.
- Deploy to Tron **only** from the fork's `main` branch.
- Keep the fork delta minimal. If upstream refactors `LibAsset` or
  `WithdrawablePeriphery`, re-check that the Tron `transferERC20` bypass and
  the `withdrawToken` re-route still apply cleanly.
...
**deploy logs are committed to upstream `contracts`, never to the fork** —
the fork receives them back through the normal sync. This keeps upstream
the single source of truth for deploy logs on every chain, Tron included.

1. **Sync the fork first.** On `contracts-tron`, bring `main` up to date
   with upstream `contracts:main` — merge the pending
   `sync/upstream-YYYY-MM-DD` PR (or open one) if upstream added or updated
   the contract you're about to deploy. (See
...
2. **Deploy from `contracts-tron`.** With the fork synced, run the Tron
   ...
   below). This writes the deploy logs — `deployments/tron.json` and
   `deployments/tron.diamond.json` — in your working tree.
3. **PR the deploy logs to _upstream_, not the fork.** Open a PR with the
   updated deploy logs directly against `lifinance/contracts` `main` — _not_
   against `contracts-tron`. Authoring the PR against upstream directly is
   why the fork still never "pushes back": the logs land in the canonical
   repo, not via a fork branch.
4. **Let the logs flow back to the fork.** Once merged upstream, those
   deploy logs reach `contracts-tron` on the next upstream→fork sync PR
   (step 1 of the next deploy).

**Net result:** code moves one-way `contracts` → `contracts-tron`; deploy
logs are written during a fork deploy, committed to upstream, then sync
back down.

### Why Tron needs custom deploy scripts

Tron requires custom deployment scripts because Foundry doesn't support it
natively. All Tron deployment scripts are located in `script/deploy/tron/`.
...
See `.agents/rules/202-tron-scripts.md` for the TypeScript conventions that
apply to everything under `script/deploy/tron/**` and `script/troncast/**`.
...
`config/global.json` → `tronWallets` (**always treat that file as the
source of truth**, do not hard-code addresses):
...
Located in `script/troncast/`, see
[script/troncast/README.md](../script/troncast/README.md) for the full
command reference. In short:
...
1. Write the Solidity contract in `src/Facets/` or `src/Periphery/`
   upstream in `contracts` first (see
   [Repo roles](#repo-roles--when-to-use-which)).
2. Compile with `forge build`.
3. Sync the fork, then create a deployment script in
   `script/deploy/tron/`:
...
6. Update `deployments/tron.json` and `deployments/tron.diamond.json`, then
   PR those logs to upstream `contracts` (step 3 of
